...
describe('getContext', () => {
  ...
  it('should transform data into correct format if all data provided', () => {
    const mockData = { organisations: [{ name: 'Greater Manchester CCG', odsCode: 'X01', orgId: 'org1' }, { name: 'Hampshire CCG', odsCode: 'X02', orgId: 'org2' }] };
    const { organisations } = getContext({ data: mockData });
    expect(organisations[0][0].data).toEqual(mockData.organisations[0].name);
    expect(organisations[0][0].href).toEqual(`organisations/${mockData.organisations[0].orgId}`);
    ...
    expect(organisations[1][0].href).toEqual(`organisations/${mockData.organisations[1].orgId}`);
