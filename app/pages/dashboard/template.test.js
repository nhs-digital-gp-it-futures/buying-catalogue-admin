...
describe('organisations dashboard page', () => {
  ...
  it('should render the table component with data', createTestHarness(setup, (harness) => {
    harness.request(context, ($) => {
      ...
      expect(row1OrgName.text().trim()).toEqual(context.organisations[0][0].data);
      expect(row1OrgName.attr('href')).toEqual(context.organisations[0][0].href);
      expect(row1OdsCode.length).toEqual(1);
      expect(row1OdsCode.text().trim()).toEqual(context.organisations[0][1].data);
      expect(row2.length).toEqual(1);
      expect(row2OrgName.length).toEqual(1);
      expect(row2OrgName.text().trim()).toEqual(context.organisations[1][0].data);
      expect(row2OrgName.attr('href')).toEqual(context.organisations[1][0].href);
      expect(row2OdsCode.length).toEqual(1);
      expect(row2OdsCode.text().trim()).toEqual(context.organisations[1][1].data);
    });
  }));
});
